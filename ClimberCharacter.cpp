...
ClimberCharacter::ClimberCharacter(int rowSet, int columnSet, Container info, ALL* allegro) : Character(rowSet, columnSet, info, allegro) {
	if(adventurerImage = al_load_bitmap_resized("Resources/Players/climberImage.png", allegro)){
		if (image = al_load_bitmap_resized("Resources/Players/Pawns/ClimberPawnImage.png", allegro)) {
			...
			takeWithMe = false;
			takeWithMeButton = new Button(allegro->screenWidth / 8, allegro->screenHeight / 2 , "Resources/Players/ClimberButton/noMouseOverCheckBoxImage.png", "Resources/Players/ClimberButton/mouseOverCheckBoxImage.png", allegro);
		}
		else { cout << "Failed to load Climber Pawn Image" << endl; }
	}												
	else { cout << "Failed to load climberImage.png" << endl; }
	//Sounds
