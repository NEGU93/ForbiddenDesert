...
bool Game::eventHandler(ALL* allegro) {
	...
	if (ev.type == ALLEGRO_EVENT_TIMER) { redraw = true; }
		...
		if (turn) { //Chech if is my turn tu play... else... do nothing... just wait ^^
			...
			case ALLEGRO_KEY_R:
				...
				break;
			case ALLEGRO_KEY_W:
				if (modeEnum == NORMAL && character2->getWaters() < MAXWATERS ) {	//TODO: cambiar cuando haga el water carrier
			...
			case ALLEGRO_KEY_O:
				if (modeEnum == NORMAL) {
					if (character->getCardIndex() == character2->getCardIndex()) {
						if (character->haveDuneBlaster() || character->haveJetPack() || character->haveSolarShield() || character->haveTerrascope() || character->haveTimeThrottle()) {
	...
	do {
		...
		case REMOVE://ANDA BIEN
			...
			if (character2->removeSand()) {
				if (tilesDeck[5 * temp.Slot2 + temp.Slot1]->haveSand()) {
					...
					SandMarkersLeft++; cout << "Sand Marker ++" << endl;
		...
		case EXCAVATE:
			if (character2->dig()) {
				...
				performTileAction(tilesDeck[character2->getCardIndex()]->gettileType(), character2->getCardIndex(), allegro);
				gamePack.ack();
				gamePack.sendPack();
				return true;
			}
...
bool Game::win(ALL *allegro) {
	...
	if (engine->getPickedState() && solarCrystal->getPickedState() && propeller->getPickedState() && navigationDeck->getPickedState()) {
		...
		if (tilesDeck[character->getCardIndex()]->gettileType() == LAUNCHPAD) {
			if (tilesDeck[character2->getCardIndex()]->gettileType() == LAUNCHPAD) {
				...
				al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight *0.4, ALLEGRO_ALIGN_CENTRE, "WE WON");
				al_flip_display();
				gamePack.wewon();
				gamePack.sendPack();
				char wait;
				...
				else if (wait == GAME_OVER) { playAgain = false; gameOver = true; }
				return true;
			}
		}
	}
		...
		if (SandMarkersLeft > 0) {
			if (character->haveWater()) {
				if (character2->haveWater()) {
					...
					al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight * 0.9, ALLEGRO_ALIGN_CENTRE, "Your partner has died from thirst");
					cout << "Your partner has died from thirst" << endl;
				}
			}
			else { 
				al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight * 0.9, ALLEGRO_ALIGN_CENTRE, "You have died from thirst");
				cout << "You have died from thirst" << endl; 
			}
		}
		else { 
			al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight * 0.9, ALLEGRO_ALIGN_CENTRE, "You habe been burried by the sand");
			cout << "You have been burried by the sand" << endl; 
		}
	}
	else {
		al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight * 0.9, ALLEGRO_ALIGN_CENTRE, "You have been killed by the storm");
		cout << "You have been killed by the storm" << endl; 
	}
	//If i'm here, it's because I lose.
	al_draw_bitmap(allegro->menuBackgroundImage, 0, 0, 0);
	al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight *0.4, ALLEGRO_ALIGN_CENTRE, "WE LOST");
	al_flip_display();
	gamePack.welost();
	gamePack.sendPack();
	char wait;
...
void Game::wannaPlay(ALL *allegro, char ID) {
	...
	if (tempQueue = al_create_event_queue()) {
		...
		playAgainButton = new Button(allegro->screenWidth * 0.25, allegro->screenHeight * 0.65, "Resources/Buttons/playAgainButton.png", "Resources/Buttons/playAgainSelectedButton.png", allegro);
		gameOverButton = new Button(allegro->screenWidth * 0.65, allegro->screenHeight * 0.65, "Resources/Buttons/gameOverButton.png", "Resources/Buttons/gameOverSelectedButton.png", allegro);
		bool run = true;
		al_draw_bitmap(allegro->menuBackgroundImage, 0, 0, 0);
		if (ID == WE_LOST) { al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight *0.4, ALLEGRO_ALIGN_CENTRE, "WE LOST"); }
		else if (ID == WE_WON) { al_draw_text(allegro->gameFont, al_map_rgb(WHITE), allegro->screenWidth / 2, allegro->screenHeight *0.4, ALLEGRO_ALIGN_CENTRE, "WE WON"); }
		while (run) {
...
pos Game::getSpot(int row, int column, ALL *allegro, ALLEGRO_BITMAP* image) {
	...
	*/
	...
	int xoffset = n;
	pos tempPos;
	int l = abs((n - al_get_bitmap_width(image) * 5) / 4);	//If the number is negative it souldn't run enyway... but at least it won't explote.
...
void Game::updateScreen(ALL* allegro) {
	...
	case OFFEREQUIPMENTMODE:
		...
		al_draw_bitmap(allegro->equipmentCards[equipOffset - 1], (allegro->screenWidth - al_get_bitmap_width(allegro->equipmentCards[equipOffset - 1])) / 2, (allegro->screenHeight - al_get_bitmap_height(allegro->equipmentCards[equipOffset - 1])) / 2, 0);
		nextButton->updateButton(allegro);
		previousButton->updateButton(allegro);
	...
	default:
		...
		int i = 0;
		if (engine->getPickedState()) { al_draw_bitmap(allegro->partsImages[i], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		else { al_draw_bitmap(allegro->partsImages[i + 4], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		i++;
		if (navigationDeck->getPickedState()) { al_draw_bitmap(allegro->partsImages[i], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		else { al_draw_bitmap(allegro->partsImages[i + 4], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		i++;
		if (propeller->getPickedState()) { al_draw_bitmap(allegro->partsImages[i], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		else { al_draw_bitmap(allegro->partsImages[i + 4], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		i++;
		if (solarCrystal->getPickedState()) { al_draw_bitmap(allegro->partsImages[i], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		else { al_draw_bitmap(allegro->partsImages[i + 4], allegro->screenWidth / 4, allegro->screenHeight / 5 + i * al_get_bitmap_height(allegro->partsImages[i]), 0); }
		//Buttons
		...
		if (MeteorologistCharacter* meteorologistCharacter = dynamic_cast<MeteorologistCharacter*>(character)) {
			...
			bool imOnTurnedWell = (tilesDeck[character->getCardIndex()]->gettileType() == WATER && tilesDeck[character->getCardIndex()]->isTurned());
		...
		if (sandStormLevel < 15) {
			al_draw_bitmap(allegro->stormMeter[sandStormLevel], allegro->screenWidth * 12 / 17, allegro->screenHeight - al_get_bitmap_height(allegro->stormMeter[sandStormLevel]), 0);
		}
		//Sand Markers Left (TEXT)
		if (SandMarkersLeft < 0) { SandMarkersLeft = 0; }
		al_draw_textf(allegro->gameFont, al_map_rgb(BLACK), allegro->screenWidth * 30 / 34, allegro->screenHeight * 25 / 32, ALLEGRO_ALIGN_LEFT, "%d", SandMarkersLeft);
	}
	if (modeEnum == OFFERWATERMODE || modeEnum == REQUESTWATERMODE) {
		al_draw_bitmap(allegro->menuBackgroundImage, 0, 0, 0);
		al_draw_text(allegro->font, al_map_rgb(WHITE), allegro->screenWidth*0.5, watersButton[0]->getInitialPos().y - 55, ALLEGRO_ALIGN_CENTRE, "How many waters?");
...
void Game::updateTilesButton(ALL* allegro) {
	...
	if (modeEnum == JETPACKMODE) {
		...
		if (modeEnum == MOVEOTHER) { 
			...
			if (turn && (tilesDeck[tempIndex]->haveSand() || !tilesDeck[tempIndex]->isTurned())) { tilesDeck[tempIndex]->updateButton(allegro); }
		...
		if (turn && tilesDeck[tempIndex]->canIMove(climber)) {
			if (modeEnum == NORMAL || modeEnum == MOVEOTHER) {
				if (tempIndex > 0 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex - 1]->canIMove(climber)) { tilesDeck[tempIndex - 1]->updateButton(allegro); } }
				if (tempIndex < TILEDECKNUMBER - 1 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex + 1]->canIMove(climber)) { tilesDeck[tempIndex + 1]->updateButton(allegro); } }
				if (tempIndex > 4) { if (tilesDeck[tempIndex - 5]->canIMove(climber)) { tilesDeck[tempIndex - 5]->updateButton(allegro); } }
				if (tempIndex < TILEDECKNUMBER - 5) { if (tilesDeck[tempIndex + 5]->canIMove(climber)) { tilesDeck[tempIndex + 5]->updateButton(allegro); } }
				if (modeEnum == NORMAL) {
					if (ExplorerCharacter* explorerCharacter = dynamic_cast<ExplorerCharacter*>(character)) {
						if (tempIndex > 5 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex - 6]->canIMove(climber)) { tilesDeck[tempIndex - 6]->updateButton(allegro); } }
						if (tempIndex > 4 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex - 4]->canIMove(climber)) { tilesDeck[tempIndex - 4]->updateButton(allegro); } }
						if (tempIndex < TILEDECKNUMBER - 4 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex + 4]->canIMove(climber)) { tilesDeck[tempIndex + 4]->updateButton(allegro); } }
						if (tempIndex < TILEDECKNUMBER - 6 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex + 6]->canIMove(climber)) { tilesDeck[tempIndex + 6]->updateButton(allegro); } }
					}
				}
			...
			if (modeEnum == REMOVESAND || modeEnum == DUNEBLASTERMODE) {
				if (tempIndex > 0 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex - 1]->haveSand()) { tilesDeck[tempIndex - 1]->updateButton(allegro); } }
				if (tempIndex < TILEDECKNUMBER - 1 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex + 1]->haveSand()) { tilesDeck[tempIndex + 1]->updateButton(allegro); } }
				if (tempIndex > 4) { if (tilesDeck[tempIndex - 5]->haveSand()) { tilesDeck[tempIndex - 5]->updateButton(allegro); } }
				if (tempIndex < TILEDECKNUMBER - 5) { if (tilesDeck[tempIndex + 5]->haveSand()) { tilesDeck[tempIndex + 5]->updateButton(allegro); } }
				if (ExplorerCharacter* explorerCharacter = dynamic_cast<ExplorerCharacter*>(character)) {
					if (tempIndex > 5 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex - 6]->haveSand()) { tilesDeck[tempIndex - 6]->updateButton(allegro); } }
					if (tempIndex > 4 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex - 4]->haveSand()) { tilesDeck[tempIndex - 4]->updateButton(allegro); } }
					if (tempIndex < TILEDECKNUMBER - 4 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex + 4]->haveSand()) { tilesDeck[tempIndex + 4]->updateButton(allegro); } }
					if (tempIndex < TILEDECKNUMBER - 6 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex + 6]->haveSand()) { tilesDeck[tempIndex + 6]->updateButton(allegro); } }
				}
			}
...
void Game::performStormCardAction(ALL *allegro, StormCardsEnum cardEnum) {
	...
	case RIGHT2:
		...
		if (i < 20) {
			...
			if (solarCrystal->getCardIndex() == i + 5) { solarCrystal->setCardIndex(i, allegro); }
			if (navigationDeck->getCardIndex() == i + 5) { navigationDeck->setCardIndex(i, allegro); }
			swapTiles(i, i + 5);			//NO need to use break right now, I make the iteration by not puting break
			tilesDeck[i]->increaseSand();
			SandMarkersLeft--;
		}
		updateScreen(allegro);
	...
	case SUNBEATSDOWN:
		...
		else if(tilesDeck[character->getCardIndex()]->gettileType() == TUNNEL && tilesDeck[character->getCardIndex()]->isTurned())
			character->addWater();
		...
		else if(tilesDeck[character2->getCardIndex()]->gettileType() == TUNNEL && tilesDeck[character2->getCardIndex()]->isTurned())
			character2->addWater();
		updateScreen(allegro);
		stormCardsDeck[0]->showImage(allegro);
		usleep(600);
...
bool Game::initializeAllegro(ALL* allegro) {
	...
	if (allegro->backgroundMusic != NULL) {
		al_play_sample(allegro->backgroundMusic, allegro->volume / 2, 0.0, 1.0, ALLEGRO_PLAYMODE_LOOP, &allegro->backgroundMusicID);
	}
...
void Game::initializeButtons(ALL* allegro) {
	modesButtons[REMOVEBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight * 0.1037, "Resources/Buttons/Actions/noMouseRemoveSand.png", "Resources/Buttons/Actions/mouseRemoveSand.png", allegro);
	modesButtons[EXCAVATEBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight * 0.1343, "Resources/Buttons/Actions/noMouseExcavate.png", "Resources/Buttons/Actions/mouseExcavate.png", allegro);
	modesButtons[PICKUPBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight / 6, "Resources/Buttons/Actions/noMousePickUpPart.png", "Resources/Buttons/Actions/mousePickUpPart.png", allegro);
	modesButtons[OFFERWBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight * 0.1981, "Resources/Buttons/Actions/noMouseOfferWater.png", "Resources/Buttons/Actions/mouseOfferWater.png", allegro);
	modesButtons[OFFEREBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight * 0.2324, "Resources/Buttons/Actions/noMouseOfferEquipment.png", "Resources/Buttons/Actions/mouseOfferEquipment.png", allegro);
	modesButtons[REQUESTWBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight * 0.2639, "Resources/Buttons/Actions/noMouseRequestWater.png", "Resources/Buttons/Actions/mouseRequestWater.png", allegro);
	modesButtons[REQUESTEBUTTON] = new Button(allegro->screenWidth * 0.68, allegro->screenHeight * 0.2972, "Resources/Buttons/Actions/noMouseRequestEquipment.png", "Resources/Buttons/Actions/mouseRequestEquipment.png", allegro);
	endTurnButton = new Button(allegro->screenWidth - al_get_bitmap_width(al_load_bitmap_resized("Resources/Buttons/noMouseEndTurnButton.png", allegro)) * 5 / 4, al_get_bitmap_height(al_load_bitmap_resized("Resources/Buttons/noMouseEndTurnButton.png", allegro)) * 3 / 2, "Resources/Buttons/noMouseEndTurnButton.png", "Resources/Buttons/mouseEndTurnButton.png", allegro);
	//allegro->volume Buttons
	muteButton = new Button(allegro->screenWidth / 4, al_get_bitmap_height(al_load_bitmap_resized("Resources/Buttons/noMouseEndTurnButton.png", allegro)) * 3 / 2, "Resources/Buttons/muteButton.png", "Resources/Buttons/muteButton.png", allegro);
	noMuteButton = new Button(allegro->screenWidth / 4, al_get_bitmap_height(al_load_bitmap_resized("Resources/Buttons/noMouseEndTurnButton.png", allegro)) * 3 / 2, "Resources/Buttons/noMuteButton.png", "Resources/Buttons/noMuteButton.png", allegro);
	// Special Buttons
	if (ALLEGRO_BITMAP* tempArrowImage = al_load_bitmap_resized("Resources/Buttons/Peep/nextPeepButton.png", allegro)) {
		previousButton = new Button(allegro->screenWidth / 2 - 300, allegro->screenHeight / 2, "Resources/Buttons/Peep/previousPeepButton.png", "Resources/Buttons/Peep/previousPeepButton.png", allegro);
		nextButton = new Button(allegro->screenWidth / 2 + 300 - al_get_bitmap_width(tempArrowImage), allegro->screenHeight / 2, "Resources/Buttons/Peep/nextPeepButton.png", "Resources/Buttons/Peep/nextPeepButton.png", allegro);
...
void Game::initializeWaterButtons(ALL* allegro) {
	// Water Buttons
	if (ALLEGRO_BITMAP* offsetImage = al_load_bitmap_resized("Resources/Buttons/Waters/1waterButtonImage.png", allegro)) {
		watersButton[2] = new Button(allegro->screenWidth *0.5 - al_get_bitmap_width(offsetImage) / 2, allegro->screenHeight* 0.5 - al_get_bitmap_height(offsetImage) / 2, "Resources/Buttons/Waters/3watersButtonImage.png", "Resources/Buttons/Waters/3watersButtonSelectedImage.png", allegro);
		watersButton[1] = new Button(watersButton[2]->getInitialPos().x, watersButton[2]->getInitialPos().y - 10 - al_get_bitmap_height(offsetImage), "Resources/Buttons/Waters/2watersButtonImage.png", "Resources/Buttons/Waters/2watersButtonSelectedImage.png", allegro);
		watersButton[0] = new Button(watersButton[2]->getInitialPos().x, watersButton[1]->getInitialPos().y - 10 - al_get_bitmap_height(offsetImage), "Resources/Buttons/Waters/1waterButtonImage.png", "Resources/Buttons/Waters/1waterButtonSelectedImage.png", allegro);
		watersButton[3] = new Button(watersButton[2]->getInitialPos().x, watersButton[2]->getInitialPos().y + 10 + al_get_bitmap_height(offsetImage), "Resources/Buttons/Waters/4watersButtonImage.png", "Resources/Buttons/Waters/4watersButtonSelectedImage.png", allegro);
		watersButton[4] = new Button(watersButton[2]->getInitialPos().x, watersButton[3]->getInitialPos().y + 10 + al_get_bitmap_height(offsetImage), "Resources/Buttons/Waters/5watersButtonImage.png", "Resources/Buttons/Waters/5watersButtonSelectedImage.png", allegro);
	}
	else { cout << "Couldn't load offset image" << endl; }
}
void Game::initializePeepButtons(ALL* allegro) {
	if (ALLEGRO_BITMAP* tempButtonImage = al_load_bitmap_resized("Resources/Buttons/Peep/putToBottomPeepImage.png", allegro)) {
		firstTimeUpdateScreenPeep = 0;
		selectPeepCard = new Button((allegro->screenWidth - al_get_bitmap_width(tempButtonImage)) / 2, allegro->screenHeight / 2 + 300, "Resources/Buttons/Peep/putToBottomPeepImage.png", "Resources/Buttons/Peep/putToBottomPeepImage.png", allegro);	
	}
...
void Game::initializeEquipmentCardButton(ALL* allegro) {
	if (ALLEGRO_BITMAP* tempButtonImage = al_load_bitmap_resized("Resources/Buttons/Peep/SelectImage.png", allegro)) {
		firstTimeUpdateScreenChooseEquipment = 0;
		selectEquipmentCard = new Button((allegro->screenWidth - al_get_bitmap_width(tempButtonImage)) / 2, allegro->screenHeight / 2 + 300, "Resources/Buttons/Peep/SelectImage.png", "Resources/Buttons/Peep/mouseOverSelectImage.png", allegro);
	}
	else { std::cout << "Failed to load Peep Button Select" << endl; }
}
//Key Methods
					...
					if (tilesDeck[character->getCardIndex()]->canIMove(climber)) {
						if (character->moveUp()) {
							if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character)) {
								if (climberCharacter->getTakeWithMe() && character2->getCardIndex() - 1 == character->getCardIndex()) {
									character2->moveUp();
						...
						if (tilesDeck[character2->getCardIndex()]->canIMove(climber)) {
							if (character2->moveUp() && navigatorCharacter->decresePartnerMoves()) {
								if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character2)) {
									if (climberCharacter->getTakeWithMe() && character->getCardIndex() - 1 == character2->getCardIndex()) {
					...
					if (tilesDeck[character2->getCardIndex() + 5]->canIMove(climber)) { //If I'm not on top... move up
						if (tilesDeck[character2->getCardIndex()]->canIMove(climber)) {
							if (character2->moveRight() && navigatorCharacter->decresePartnerMoves()) {
								if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character2)) {
									if (climberCharacter->getTakeWithMe() && character->getCardIndex() + 5 == character2->getCardIndex()) {
										character->moveRight();
									}
								}
								gamePack.requestmove(character2->getRow(), character2->getColumn());
		...
		if (!(tilesDeck[character->getCardIndex()]->isTurned())) {
			if (!tilesDeck[character->getCardIndex()]->haveSand()) {
					if (character->dig()) {
						...
						performTileAction(tilesDeck[character->getCardIndex()]->gettileType(), character->getCardIndex(), allegro);
						gamePack.excavate();
						gamePack.sendPack();
						gamePack.waitForAck(allegro);
					}
...
void Game::chooseOption(ALL *allegro, Container temp) {
	...
	if (tempQueue = al_create_event_queue()) {
		...
		if (ALLEGRO_BITMAP* offsetImage = al_load_bitmap_resized("Resources/Buttons/agreeButton.png", allegro)) {
			disagreeButton = new Button((int)allegro->screenWidth *0.55, (int)allegro->screenHeight*0.52, "Resources/Buttons/disagreeButton.png", "Resources/Buttons/disagreeButtonSelected.png", allegro);
			agreeButton = new Button((int)allegro->screenWidth *0.45 - al_get_bitmap_width(offsetImage), (int)allegro->screenHeight*0.52, "Resources/Buttons/agreeButton.png", "Resources/Buttons/agreeButtonSelected.png", allegro);
			al_destroy_bitmap(offsetImage);
			al_draw_bitmap(allegro->menuBackgroundImage, 0, 0, 0);
			switch (temp.ID) {
			case OFFER_WATER: case REQUEST_WATER:
				if (temp.ID == OFFER_WATER) al_draw_textf(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer %d waters", temp.Slot1);
				else al_draw_textf(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request %d waters", temp.Slot1);
				break;
			case OFFER_EQUIPMENT: case REQUEST_EQUIPMENT:
				switch (temp.Slot1) {
				case DUNEBLAST:
					if (temp.ID == OFFER_EQUIPMENT) al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer a duneblast");
					else al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request a duneblast");
					break;
				case JETPACK:
					if (temp.ID == OFFER_EQUIPMENT) al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer a jetpack");
					else al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request a jetpack");
					break;
				case SOLARSHIELD:
					if (temp.ID == OFFER_EQUIPMENT) al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer a solarshield");
					else al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request a solarshield");
					break;
				case TERRASCOPE:
					if (temp.ID == OFFER_EQUIPMENT) al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer a terrascope");
					else al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request a terrascope");
					break;
				case SECRETWATERRESERVE:
					if (temp.ID == OFFER_EQUIPMENT) al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer a secret water reserve");
					else al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request a secret water reserve");
					break;
				case TIMETHROTTLE:
					if (temp.ID == OFFER_EQUIPMENT) al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Offer a time throttle");
					else al_draw_text(allegro->font, al_map_rgb(WHITE), 0.5*allegro->screenWidth, 0.45*allegro->screenHeight, ALLEGRO_ALIGN_CENTRE, "Request a time throttle");
					break;
				}
				break;
			}
			...
			while (run) {
				...
				if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) {
					if (agreeButton->checkMouse(mouse.x, mouse.y, allegro->volume)) {
						...
						modeEnum = NORMAL;
						if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character)) { 
							al_play_sample(allegro->rejectWaterBoySound, allegro->volume, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);	//TODO: HAY FORMA DE SABER SI ES WATER O EQUIP?
						}
...
void Game::clickOnPeepMode(ALL* allegro) {
	...
	if (MeteorologistCharacter* meteorologistCharacter = dynamic_cast<MeteorologistCharacter*>(character)) {
		...
		if (selectPeepCard->checkMouse(mouse.x, mouse.y, allegro->volume)) {
			for (int i = 0; i < peepOffset; i++) { swapStormCards(peepOffset - i - 1, peepOffset - i); } //Move the card to the front by saping with each card
			std::rotate(stormCardsDeck.begin(), stormCardsDeck.begin() + 1, stormCardsDeck.end());		//Rotate to take it to the bottom.
...
void Game::clickOnOfferEquipmentMode(ALL* allegro) {
	...
	if (selectEquipmentCard->checkMouse(mouse.x, mouse.y, allegro->volume)) {
		...
		if (wait == AGREE) {
			...
			if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character2)) {
				al_play_sample(allegro->rejectEquipmentBoySound, allegro->volume, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);	//TODO: HAY FORMA DE SABER SI ES WATER O EQUIP?
			}
			if (ArcheologistCharacter* archeologistWater = dynamic_cast<ArcheologistCharacter*>(character2)) {
...
void Game::clickOnDefaultMode(ALL* allegro) {
	...
	if (endTurnButton->checkMouse(mouse.x, mouse.y, allegro->volume)) {			//If there whas a selected character, then return true, else return false
...
void Game::checkMouse(ALL* allegro) {
	...
	case TERRASCOPEMODE:
		...
		break;
	case OFFEREQUIPMENTMODE: case REQUESTEQUIPMENTMODE:
		nextButton->checkMouse(mouse.x, mouse.y, allegro->volume);
		previousButton->checkMouse(mouse.x, mouse.y, allegro->volume);
		selectEquipmentCard->checkMouse(mouse.x, mouse.y, allegro->volume);
		break;
	case OFFERWATERMODE: case REQUESTWATERMODE:
		for (int i = 0; i < min(character->getWaters(), MAXWATERS - character2->getWaters() - 1); i++) {
	...
	default:
		...
		for (int i = 0; i < MODEBUTTONSNUMBER; i++) { modesButtons[i]->checkMouse(mouse.x, mouse.y, allegro->volume); }
...
void Game::clickModesButtons(ALL* allegro) {
	if (modesButtons[REMOVEBUTTON]->checkMouse(mouse.x, mouse.y, allegro->volume)) {
	...
	else if (modesButtons[EXCAVATEBUTTON]->checkMouse(mouse.x, mouse.y, allegro->volume)) {	//I use else if because it will take less time for the computer to check the buttons
		...
		if (WaterCarrierCharacter* waterCarrierCharacter = dynamic_cast<WaterCarrierCharacter*>(character)) {
			if (character->getRow() == character2->getRow()) {
				if (character2->getColumn() >= character->getColumn() - 1 && character2->getColumn() <= character->getColumn() + 1) {
					if (min(character->getWaters(), MAXWATERS - character2->getWaters() - 1) != 0) { modeEnum = OFFERWATERMODE; }
				}
			}
		...
		if (WaterCarrierCharacter* waterCarrierCharacter = dynamic_cast<WaterCarrierCharacter*>(character2)) {
			if (character->getRow() == character2->getRow()) {
				if (character->getColumn() >= character2->getColumn() - 1 && character->getColumn() <= character2->getColumn() + 1) {
					if (min(character2->getWaters(), MAXWATERS - character->getWaters() - 1) != 0) { modeEnum = REQUESTWATERMODE; }
				}
			}
			else if (character->getColumn() == character2->getColumn()) {
				if (character2->getColumn() == character->getColumn() - 1 || character2->getColumn() == character->getColumn() + 1) {
		...
		al_play_sample(allegro->clickOnButtonSound, allegro->volume, 0, 1, ALLEGRO_PLAYMODE_ONCE, NULL);
		if (character->getCardIndex() == character2->getCardIndex()) {
			if (character2->haveDuneBlaster() || character2->haveJetPack() || character2->haveSolarShield() || character2->haveTerrascope() || character2->haveTimeThrottle()) {
...
void Game::checkTilesButtons(double volume) { //TODO: cuando el jetPack actualiza todos los tiles tendría que ser acá.
	...
	if (modeEnum == MOVEOTHER) { 
		...
		tempIndex = character->getCardIndex();
		if (turn && (tilesDeck[tempIndex]->haveSand() || !tilesDeck[tempIndex]->isTurned())) { tilesDeck[tempIndex]->checkButton(mouse.x, mouse.y, volume); }
		if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character)) { climber = true; }
	}
	if (turn && tilesDeck[tempIndex]->canIMove(climber)) {
		if (modeEnum == NORMAL || modeEnum == MOVEOTHER) {
			if (tempIndex > 0) { if (tilesDeck[tempIndex - 1]->canIMove(climber) && tempIndex % 5 != 0) { tilesDeck[tempIndex - 1]->checkButton(mouse.x, mouse.y, volume); } }
			if (tempIndex < TILEDECKNUMBER - 1) { if (tilesDeck[tempIndex + 1]->canIMove(climber) && tempIndex % 5 != 4) { tilesDeck[tempIndex + 1]->checkButton(mouse.x, mouse.y, volume); } }
			if (tempIndex > 4) { if (tilesDeck[tempIndex - 5]->canIMove(climber)) { tilesDeck[tempIndex - 5]->checkButton(mouse.x, mouse.y, volume); } }
			if (tempIndex < TILEDECKNUMBER - 5) { if (tilesDeck[tempIndex + 5]->canIMove(climber)) { tilesDeck[tempIndex + 5]->checkButton(mouse.x, mouse.y, volume); } }
			//Tunnel Tile
			if (tilesDeck[tempIndex]->gettileType() == TUNNEL && tilesDeck[tempIndex]->isTurned()) {
				for (uint i = 0; i < tilesDeck.size(); i++) {
					if (tilesDeck[i]->gettileType() == TUNNEL && i != tempIndex && tilesDeck[i]->isTurned()) {
			...
			if (modeEnum == NORMAL) {
				if (ExplorerCharacter* explorerCharacter = dynamic_cast<ExplorerCharacter*>(character)) { //TODO: fix this for character2
					if (tempIndex > 5 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex - 6]->canIMove(climber)) { tilesDeck[tempIndex - 6]->checkButton(mouse.x, mouse.y, volume); } }
					if (tempIndex > 5 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex - 4]->canIMove(climber)) { tilesDeck[tempIndex - 4]->checkButton(mouse.x, mouse.y, volume); } }
					if (tempIndex < TILEDECKNUMBER - 4 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex + 4]->canIMove(climber)) { tilesDeck[tempIndex + 4]->checkButton(mouse.x, mouse.y, volume); } }
					if (tempIndex < TILEDECKNUMBER - 6 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex + 6]->canIMove(climber)) { tilesDeck[tempIndex + 6]->checkButton(mouse.x, mouse.y, volume); } }
				}
			}
			else if (modeEnum == MOVEOTHER) {
				if (ExplorerCharacter* explorerCharacter = dynamic_cast<ExplorerCharacter*>(character2)) {
		...
		if (modeEnum == REMOVESAND || modeEnum == DUNEBLASTERMODE) { //I pass always true because in this mode I only don't want not to be marked if its the storm
			if (tempIndex > 0) { if (tilesDeck[tempIndex - 1]->haveSand() && tempIndex % 5 != 0) { tilesDeck[tempIndex - 1]->checkButton(mouse.x, mouse.y, volume); } }
			if (tempIndex < TILEDECKNUMBER - 1) { if (tilesDeck[tempIndex + 1]->haveSand() && tempIndex % 5 != 4) { tilesDeck[tempIndex + 1]->checkButton(mouse.x, mouse.y, volume); } }
			if (tempIndex > 4) { if (tilesDeck[tempIndex - 5]->haveSand()) { tilesDeck[tempIndex - 5]->checkButton(mouse.x, mouse.y, volume); } }
			if (tempIndex < TILEDECKNUMBER - 5) { if (tilesDeck[tempIndex + 5]->haveSand()) { tilesDeck[tempIndex + 5]->checkButton(mouse.x, mouse.y, volume); } }
			// Explorer Move Diagonally
			if (ExplorerCharacter* explorerCharacter = dynamic_cast<ExplorerCharacter*>(character)) {
				if (tempIndex > 5 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex - 6]->haveSand()) { tilesDeck[tempIndex - 6]->checkButton(mouse.x, mouse.y, volume); } }
				if (tempIndex > 5 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex - 4]->haveSand()) { tilesDeck[tempIndex - 4]->checkButton(mouse.x, mouse.y, volume); } }
				if (tempIndex < TILEDECKNUMBER - 4 && tempIndex % 5 != 0) { if (tilesDeck[tempIndex + 4]->haveSand()) { tilesDeck[tempIndex + 4]->checkButton(mouse.x, mouse.y, volume); } }
				if (tempIndex < TILEDECKNUMBER - 6 && tempIndex % 5 != 4) { if (tilesDeck[tempIndex + 6]->haveSand()) { tilesDeck[tempIndex + 6]->checkButton(mouse.x, mouse.y, volume); } }
			}
		}
	}
}
void Game::clickTilesButtons(ALL *allegro) {
	if (modeEnum != MOVEOTHER) {
		...
		if (turn && (tilesDeck[character->getCardIndex()]->haveSand() || !tilesDeck[character->getCardIndex()]->isTurned())) {
			if (tilesDeck[character->getCardIndex()]->checkButton(mouse.x, mouse.y, allegro->volume)) {
		...
		bool climber = false;
		if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character)) { climber = true; }
		if (tilesDeck[character->getCardIndex()]->gettileType() == TUNNEL && tilesDeck[character->getCardIndex()]->isTurned()) {
			for (uint i = 0; i < tilesDeck.size(); i++) {
				if (tilesDeck[i]->gettileType() == TUNNEL && i != character->getCardIndex() && tilesDeck[i]->isTurned()) { //TUNNEL not the same I'm in, it's discovered
					if (tilesDeck[i]->canIMove(climber)) { 
						if (tilesDeck[i]->checkButton(mouse.x, mouse.y, allegro->volume)) {
							character->setPos(i % 5, i / 5);
							gamePack.move(character->getRow(), character->getColumn());
		...
		if (ExplorerCharacter* explorerCharacter = dynamic_cast<ExplorerCharacter*>(character)) {
			...
			if (character->getCardIndex() > 5 && character->getCardIndex() % 5 != 0) {
				if (tilesDeck[character->getCardIndex() - 6]->checkButton(mouse.x, mouse.y, allegro->volume)) {
					if (modeEnum == NORMAL) {
						if (tilesDeck[character->getCardIndex() - 6]->canIMove(climber)) {	//Here climber will always be false
		...
		bool climber = false;
		if (ClimberCharacter* climberCharacter = dynamic_cast<ClimberCharacter*>(character2)) { climber = true; }
		if (tilesDeck[character2->getCardIndex()]->gettileType() == TUNNEL && tilesDeck[character2->getCardIndex()]->isTurned()) {
			for (uint i = 0; i < tilesDeck.size(); i++) {
				if (tilesDeck[i]->gettileType() == TUNNEL && i != character2->getCardIndex() && tilesDeck[i]->isTurned()) {
					if (tilesDeck[i]->canIMove(climber)) {
						if (tilesDeck[i]->checkButton(mouse.x, mouse.y, allegro->volume)) {
							character2->setPos(i % 5, i / 5);
							gamePack.requestmove(character2->getRow(), character2->getColumn());
			...
			if (NavigatorCharacter* navigatorCharacter = dynamic_cast<NavigatorCharacter*>(character)) {
				...
				if (character2->getCardIndex() > 5 && character2->getCardIndex() % 5 != 0) {
					if (tilesDeck[character2->getCardIndex() - 6]->checkButton(mouse.x, mouse.y, allegro->volume)) {
						if (navigatorCharacter->havePartnersMovesLeft()) {
							if (tilesDeck[character2->getCardIndex() - 6]->canIMove(climber)) {	//Here climber will always be false
								explorerCharacter->moveUpLeftDiagonally();
